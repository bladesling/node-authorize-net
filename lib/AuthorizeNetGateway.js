    ...
    .spread(createJsonCallback(function (json) {
      if (json.createTransactionResponse) {
        ...
        if (json.createTransactionResponse.messages.resultCode === 'Error') {
          if( typeof json.createTransactionResponse.transactionResponse.errors.error.errorText === 'string' ) {
            ...
            var errors = json.createTransactionResponse.transactionResponse.errors.error;
            var errorText = '';
            for (var i = 0; i < errors.length; i++) {
                errorText += errors[i].errorText + ' ';
            }
            throw new GatewayError(errorText, json.createTransactionResponse);
          }
        }
